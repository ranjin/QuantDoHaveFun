...
- (void)layoutSubviews{
    ...
    [_dealLab mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(_shadowView.mas_left).offset(SCREEN_WIDTH*0.5);
    }];
...
- (void)loadPurchaseDataWithModel:(BiddingPostersDTO *)DTO withTag:(NSInteger)btnTag{
    ...
    _deal.text =  [NSString stringWithFormat:@"%@个",DTO.tradedVolume];
    _frozen.text = [NSString stringWithFormat:@"%@个",DTO.frozenVolume];
    //剩余数量
    if (DTO.volume == nil) {
        self.amount.text = @"--";
    }else{
